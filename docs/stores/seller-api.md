...
4. Скопируйте значение и вставьте в поле **Seller API Client ID** в карточке магазина в STPulse.
...
2. Укажите любое понятное имя — например, `STPulse Seller` или `STPulse аналитика`.
...
5. Скопируйте ключ **сразу** и вставьте в поле **Seller API Key** в STPulse.
...
!!! example "Пример"
    ...
    2. Нажала «Создать ключ», назвала его `STPulse-Анна-Декор`, выбрала **Admin read only**.
    3. Скопировала сгенерированный ключ (длинная строка из 64 символов). Закрыла окно.
    4. В STPulse в карточке магазина вставила: Client ID `12345678` и Key — скопированный.
...
Один ключ на магазин. В отличие от Performance API (где 5 ключей ускоряют рекламу), Seller API имеет более мягкие лимиты — одного ключа достаточно.
...
Создайте новые ключи Seller API в обновлённом кабинете Ozon, обновите их в карточке магазина STPulse. Старые данные останутся, новые начнут приходить с новых ключей.

**Что считается «инвалидным ключом»?**
Если STPulse вернёт ошибку «ключ недействителен», возможные причины: ключ удалён в Ozon, выбрана неправильная роль, ключ скопирован не полностью, или Ozon отключил ключ за неактивность. Проверьте по этому списку и при необходимости создайте новый.

---

Подключение Seller API — самый ответственный шаг настройки магазина. От правильности этих ключей зависит, увидите ли вы свои финансы и продажи в STPulse. После подключения переходите к [Performance API](performance-api.md) для рекламной аналитики.
